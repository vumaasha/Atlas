...
class Parivar:
    def __init__(self):
        ...
        self.source_urls_col = 'parivar'  # Column name having the source URL's in CSV file
    ...
    def parse_items(self,source_url_list, taxonomy_list):
        try:
            ...
            for url,tax in zip(source_url_list,taxonomy_list):
                self.driver.get(url)
                print("got url")
                p_element = self.driver.find_elements_by_css_selector('li.item.last a.product-image')
                for e in p_element:
