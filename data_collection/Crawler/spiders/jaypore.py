...
class Jaypore:
    def __init__(self):
        ...
        self.source_urls_col = 'jaypore'  # Column name having the source URL's in CSV file
    ...
    def parse_items(self,source_url_list, taxonomy_list):
        try:
            ...
            for url,tax in zip(source_url_list,taxonomy_list):
                self.driver.get(url)
                print("got url")
                p_element = self.driver.find_elements_by_css_selector('a#prdName')
                for e in p_element:
    ...
    def parse_product(self,each_url,taxonomy):
        try:
            ...
            web_image_url = self.driver.find_elements_by_css_selector('div#th1big img')[0].get_attribute('src')
            ...
            product_details['product_price'] = 'Rs.'+self.driver.find_elements_by_css_selector('span#dPrice')[0].text
            product_details['source'] = 'Jaypore'
            product_details['product_title'] = self.driver.find_elements_by_css_selector('h1.productName')[0].text
            product_details['taxonomy'] = taxonomy.replace("->","/")
            product_details['product_info'] = self.driver.find_elements_by_css_selector('span#prodDesc')[0].text
