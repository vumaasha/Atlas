...
def get_product_data_with_split(source_folder,categories, train_split_val=0.70, val_split_val=0.20):
    ...
    for category in categories:
        ...
        images_list = glob.glob(source_folder + 'atlas_dataset/' + category + '/images/*.jp*', recursive=True)
        with open(source_folder + 'atlas_dataset/' + category + '/data.json', 'r') as f:
        ...
        product_data = list(filter(lambda x: source_folder + x['filename'] in images_list, cleaned_json_data))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--destination_path', '-dp', help='Destination path to atlas_dataset.json file')
    ...
    categories = os.listdir(source_folder+'/atlas_dataset')
    ...
    with open(dest_folder+json_file_name+'.json', 'w') as outfile:
