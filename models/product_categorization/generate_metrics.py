...
def get_predictions(karpathy_json_path, model_path, word_map_path, beam_size, predictions_file_path):
    ...
    for img in tqdm(data['images']):
        img_path = img['filename'].replace("/home/shinchan/atlas_datasets/", "../")
...
def get_metrics_from_predictions(predictions_file_path):
    ...
    metrics_df.to_csv("train_data_metrics.csv")
    ...
    print(classification_report(y_true, y_pred))
    metrics_df = pd.DataFrame(report2dict(classification_report(y_true, y_pred))).T
    metrics_df.to_csv("test_data_metrics.csv")


if __name__ == '__main__':
    get_predictions(karpathy_json_path='../coresdataset19/coresdataset19.json',
                    model_path='BEST_checkpoint_flickr8k_1_cap_per_img_1_min_word_freq.pth.tar',
                    word_map_path='../output/WORDMAP_flickr8k_1_cap_per_img_1_min_word_freq.json',
                    beam_size=5,
                    predictions_file_path='model_predictions_cbs.csv'
                    )
    get_metrics_from_predictions(predictions_file_path='model_predictions_cbs.csv')
